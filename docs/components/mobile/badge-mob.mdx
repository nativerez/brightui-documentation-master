...
- To present information, such as status, category or quantity of inormation in a section.
- When items need to be differentiated from each other.

#### When not to use

- If the entire message has an associated action—use an alert.
- For a call to action, use a button instead.
- If the indicator can be removed or replaces, use a tag instead.
...
Example: Absence request can display if they have been approved or not.
...
Numbered badges are used to indicate the quantity of items. For example this can be used in an accordian to indicate the number of items that will appear when an accordian is opened.

These badges are not the same as notification badges and do not disappear when content has been viewed. Numbered badges should always be neutral or info variants and none of the other variants listed under badge colours.
